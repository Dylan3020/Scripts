if not getgenv().doorsrooms_loaded then
    getgenv().doorsrooms_loaded = true
else return end
...
if not isRooms then
    ...
    remotesFolder = ReplicatedStorage:WaitForChild("Bricks")
end
...
local _doorsrooms_custom_captions = Instance.new("ScreenGui") do
    local Frame = Instance.new("Frame", _doorsrooms_custom_captions)
    local TextLabel = Instance.new("TextLabel", Frame)
    local UITextSizeConstraint = Instance.new("UITextSizeConstraint", TextLabel)

    _doorsrooms_custom_captions.Parent = ReplicatedStorage
    _doorsrooms_custom_captions.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ...
    function Script.Functions.Captions(caption: string)
        if _doorsrooms_custom_captions.Parent == ReplicatedStorage then _doorsrooms_custom_captions.Parent = gethui() or game:GetService("CoreGui") or playerGui end
...
getgenv()._internal_unload_doorsrooms = function()
    Library:Unload()
end
        ...
        if character then
            Script.Connections["AnticheatBypassTheMines"] = character:GetAttributeChangedSignal("Climbing"):Connect(function()                                
                if Toggles.TheMinesAnticheatBypass.Value and character:GetAttribute("Climbing") then
                    ...
                    if workspace:FindFirstChild("_internal_doorsrooms_acbypassprogress") then workspace:FindFirstChild("_internal_doorsrooms_acbypassprogress"):Destroy() end
                end
            end)
        end
...
task.spawn(function()
        ...
        Toggles.TheMinesAnticheatBypass:OnChanged(function(value)
            if value then
                local progressPart = Instance.new("Part", workspace) do
                    ...
                    progressPart.Name = "_internal_doorsrooms_acbypassprogress"
    ...
    elseif isBackdoor then
        ...
        Toggles.AntiHasteJumpscare:OnChanged(function(value)
            ...
            local internal_temp_doorsrooms = clientRemote:FindFirstChild("_doorsrooms")
            
            if not internal_temp_doorsrooms then internal_temp_doorsrooms = Instance.new("Folder", clientRemote); internal_temp_doorsrooms.Name = "_doorsrooms" end

            if value then
                for i,v in pairs(clientRemote.Haste:GetChildren()) do
                    if v:IsA("RemoteEvent") then continue end

                    v.Parent = internal_temp_doorsrooms
                end
            else
                for i,v in pairs(internal_temp_doorsrooms:GetChildren()) do
    ...
    elseif isRooms then
        ...
        local _internal_doorsrooms_pathfinding_nodes = Instance.new("Folder", workspace) do
            _internal_doorsrooms_pathfinding_nodes.Name = "_internal_doorsrooms_pathfinding_nodes"
        end

        Toggles.ShowAutoRoomsPathNodes:OnChanged(function(value)
            for _, node in pairs(_internal_doorsrooms_pathfinding_nodes:GetChildren()) do
            ...
            if value then
                ...
                local function doAutoRooms()
                    ...
                    if path.Status == Enum.PathStatus.Success then
                        ...
                        _internal_doorsrooms_pathfinding_nodes:ClearAllChildren()

                        for i, waypoint in pairs(waypoints) do
                            local node = Instance.new("Part", _internal_doorsrooms_pathfinding_nodes) do
                        ...
                        for i, waypoint in pairs(waypoints) do
                            ...
                            if not Toggles.AutoRooms.Value then
                                ...
                                if _internal_doorsrooms_pathfinding_nodes:FindFirstChild("_internal_node_" .. i) then
                                    _internal_doorsrooms_pathfinding_nodes:FindFirstChild("_internal_node_" .. i):Destroy()
                                end
                            end
    ...
    Library:GiveSignal(clientRemote.Haste.Remote.OnClientEvent:Connect(function(value)
        if not value and Toggles.NotifyEntity.Value then
            haste_incoming_progress = Instance.new("Part", workspace) do
                ...
                haste_incoming_progress.Name = "_internal_doorsrooms_haste"
...
Library:OnUnload(function()
    ...
    if isBackdoor then
        ...
        if internal_temp_doorsrooms and #internal_temp_doorsrooms:GetChildren() ~= 0 then
            for i,v in pairs(internal_temp_doorsrooms:GetChildren()) do
                v.Parent = clientRemote.Haste
            end
        end

        internal_temp_doorsrooms:Destroy()
    end

    if isRooms then
        if workspace:FindFirstChild("_internal_doorsrooms_pathfinding_nodes") then
            workspace:FindFirstChild("_internal_doorsrooms_pathfinding_nodes"):Destroy()
        end
    end

    if _doorsrooms_custom_captions then
        _doorsrooms_custom_captions:Destroy()
    end
